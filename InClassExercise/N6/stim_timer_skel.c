  ...
  while(1) {
	...
	break;
      ...
      case SR_RANDOM_DELAY: {
        ...
        // Set up TC0
       TCCR0 |= (1<<CS00);// set TC0 timer into normal mode and disable clock
    ...
    case SR_TIMING_USER: {
      ...
      TCCR1B |= (0<< ICNC1)|(0<<WGM13)| (0<<WGM12)|(0<<CS12)|(0<<CS11)|(0<<CS10);// disable noise canceler, set WGM1{3,2} to 0, and disable clock
      TCCR1A |= (0<<COM1A1) |(0<<COM1A0)|(0<<WGM11)|(0<<WGM10);// disable all of the output compare pins and set WGM1{1,0} to 0
      TIMSK |= (0<<TOIE1)|(0<<OCIE1A);// disable TC1 interrupts in TIMSK
     ...
     TCCR1B |= (1<<CS12)|(1<<CS10);                                          // start TC1 counter
      ...
      numticks = TCNT1;
     TCCR1B |= (0<<CS12)|(1<<CS10);                    // stop the TC1 counter
